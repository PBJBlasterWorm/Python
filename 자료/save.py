...
class Mainmenu(tk.Tk):
    ...
    def show_menu(self):
        ...
        label = tk.Label(self.current_frame, text="select mode", font=("맑은고딕", 20))
        label.pack(pady=40)
        
        pvp_button = tk.Button(self.current_frame, text="PvP Mode", command=self.start_pvp_game)
        pvp_button.pack(pady=20)
        
        pve_button = tk.Button(self.current_frame, text="PvE Mode", command=self.start_pve_game)
...
class OmokGame:
    ...
    def start_pve_game(self):
        self.restart_game()
        self.canvas.bind("button-1", self.ai_move)
