...
class OmokGame:
    ...
    def ai_move(self, row=None, col=None):
        if not self.game_over:
            # AI가 놓을 수 있는 후보 위치를 계산합니다.
            candidate_positions = []
            for i in range(max(0, row - 1), min(self.board_size, row + 2) if row is not None else self.board_size):  # 행 범위를 늘려 검사합니다.
                for j in range(max(0, col - 1), min(self.board_size, col + 2) if col is not None else self.board_size):  # 열 범위를 늘려 검사합니다.
                    if self.board[i][j] == 0 and not self.is_overlapping(i, j):
    ...
    def is_overlapping(self, row, col):
        for dr in range(-1, 2):
            for dc in range(-1, 2):
                if dr == dc == 0:
                    continue
                if (row + dr, col + dc) in self.stone_positions:
                    return True
        return False
